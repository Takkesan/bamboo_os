...
// efi_main() の第 2 引数から EFI System Table のアドレスが得られる
// EFI System Table の中に locate_protocol() という関数がある
// UEFI仕様書[uefi_2_11]にEFI Graphics Output Protocolに対応するGUIDが定義されている
// このGUID をlocate_protocol()に渡してあげればEFI Graphics Output Protocol へのポインタを取得できる
// EFI Graphics Output Protocol 構造体の中に、フレームバッファのアドレスなどの情報がある
// フレームバッファのアドレスを取得して、そこに色を塗る
...
#[unsafe(no_mangle)]
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) -> EfiStatus {
